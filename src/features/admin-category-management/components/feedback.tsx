...
import type { GuardFeedback } from "../category-safeguards";
...
export function FailureMessage({ kind, action, onRetry }: { kind: FailureKind; action: string; onRetry?: () => void }) {
  const messages: Record<FailureKind, string> = {
    ...
    malformed: "The server returned an invalid Service Category response.",
    offline: `You are offline. Cannot ${action} this Service Category.`,
    network: "Network error. Please try again.",
    server: "Server error. Please try again later.",
    not_found: "Service Category not found.",
    forbidden: "Access forbidden.",
    unauthorized: "Your session is not authorized for this action.",
    validation: "The server rejected the Service Category name.",
