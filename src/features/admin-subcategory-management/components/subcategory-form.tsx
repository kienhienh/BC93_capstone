...
import { normalizeSubcategoryName, validateSubcategoryName } from "../validation";

export function SubcategoryForm({ initialName, pending, submitLabel, idPrefix, cancelTo, serverNameError, onClearServerNameError, onSubmit }: {
  initialName: string;
  pending: boolean;
  submitLabel: string;
  idPrefix: string;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLocalError(null);
    if (await onSubmit(normalizeSubcategoryName(name))) setDirty(false);
  };

  return <>
    <form className="admin-category-form" onSubmit={submit} noValidate aria-busy={pending}>
      <div className="form-field">
        <label htmlFor={`${idPrefix}-name`}>Service Subcategory name *</label>
