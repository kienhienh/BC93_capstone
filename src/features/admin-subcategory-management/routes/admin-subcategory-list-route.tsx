...
import { useAdminSubcategoryList } from "../controller";
...
export function AdminSubcategoryListRoute() {
  ...
  const listQuery = useAdminSubcategoryList({ pageIndex: page, pageSize, keyword: q || undefined }, session?.token ?? "");
  ...
  return <main id="main-content" className="admin-subcategory-list">
    ...
    <header className="admin-page-heading admin-subcategory-heading-row">
      <div><span className="admin-eyebrow">Marketplace taxonomy</span><h1 ref={heading} tabIndex={-1}>Service Subcategory Management</h1>
        <p>Manage the selectable Service Subcategory leaves. Service Group membership is edited from a Service Category.</p></div>
    ...
    {listState === "confirmed-success" && listQuery.data ? <p className="admin-subcategory-list-scope" data-scope={listQuery.data.scope}>
      {listQuery.data.scope === "server"
        ? "Service Subcategory results are paginated and filtered by the Subcategory API."
        : "The paging response was unusable, so this page is a client-filtered view of the complete Subcategory API snapshot."}
    </p> : null}

    <div className="admin-subcategory-list-controls">
      <div><label htmlFor="subcategory-search">Search Service Subcategories</label>
      ...
      <div><label htmlFor="subcategory-page-size">Page size</label>
    ...
    {listState === "confirmed-success" && listQuery.data ? <div className="admin-subcategory-table">
      <table role="grid" aria-label="Service Subcategory list">
        <thead><tr><th scope="col">Service Subcategory</th><th scope="col">Identifier</th><th scope="col">Actions</th></tr></thead>
        <tbody>{listQuery.data.data.map((subcategory) => <tr key={subcategory.id}>
          <td data-label="Service Subcategory"><strong>{subcategory.name}</strong></td>
          <td data-label="Identifier">{subcategory.id}</td>
