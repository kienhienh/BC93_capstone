import { AdminSubcategoryManagementFailure } from "./capability";

export const VALID_PAGE_SIZES = [10, 25, 50] as const;
export const DEFAULT_PAGE_SIZE = 10;
export type FailureKind = AdminSubcategoryManagementFailure["kind"] | "unknown";
...
export function kindOf(error: unknown): FailureKind {
  return error instanceof AdminSubcategoryManagementFailure ? error.kind : "unknown";
}

export function listPath(search: string) {
  return `/admin/subcategories${search}`;
}
